...
clewd_subversion=$(curl -s https://gh.llkk.cc/https://raw.githubusercontent.com/teralomaniac/clewd/test/lib/clewd-utils.js | grep "Main = 'clewd修改版 v'" | awk -F'[()]' '{print $3}')
...
function TavernAI-extrasinstall {
	...
	if [ ! -d "/fox/TavernAI-extras" ]; then
		...
		[ -d /fox/TavernAI-extras ] || { echo "TavernAI-extras（酒馆拓展）安装失败，请更换网络后重试喵~"; exit 8; }
	fi
